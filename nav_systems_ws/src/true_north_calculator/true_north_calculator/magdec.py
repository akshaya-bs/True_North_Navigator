...
class GeoMag:
    ...
    def _get_coefficients_year(cls, year: Union[str, datetime.datetime]) -> str:
        ...
        if 2025 <= year_value < 2030:  # noqa: PLR2004 Magic value used in comparison
            ...
            raise ValueError(
                f"There are no coefficients for the year {year_value}")
    ...
    def _get_model_filename(self) -> str:
        ...
        if self._coefficients_file:
            return os.path.join(filepath, self._coefficients_file)
        ...
        coefficients_file = f"wmm{os.sep}WMM{hr}{year}.COF"
        wmm_filepath = os.path.join(filepath, coefficients_file)
        try:
        ...
        wmm_filepath_2 = os.path.join(filepath, coefficients_file)
        try:
...
geo_mag = GeoMag(coefficients_file="WMM.COF")
date = datetime.datetime(2026, 2, 20)
decimal_year = TimeConverter.decimal_year_from_date(date)
result = geo_mag.calculate(
    glat=1,  # latitude (degrees)
    glon=-1,  # longitude (degrees)
    alt=1,  # altitude (km)
    time=decimal_year# decimal year
)

# LATITUDE North positive, South negative)
#LONGITUDE East positive, West negative
# 3. Determine East or West Declination
if result.d > 0:
    declination_direction = "East"
    true_north_direction = "left" # True north is to the left of magnetic north
elif result.d < 0:
    declination_direction = "West"
    true_north_direction = "right" # True north is to the right of magnetic north
else:
    declination_direction = "Zero"
    true_north_direction = "aligned"  # True north and magnetic north are aligned (rare)


print(f"Magnetic Declination: {result.d} degrees {declination_direction}")
print(f"True North is to the {true_north_direction} of Magnetic North.")
